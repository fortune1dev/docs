...
By default, Fortune1Coin Node Service (`fortune1coind`, Node Daemon) is bound only to `127.0.0.1` (`localhost`) interface, so it can be accessed only from the same computer it runs on. This is done to reduce number of external attack vectors. `fortune1coind` itself has access to only public information, but it sometimes runs in the same process with `walletd`, which has access to wallet keys. To bind `fortune1coind` to all network interfaces, use `--fortune1coind-bind-address=0.0.0.0:8888` command line argument (specifying port is mandatory).

To make a JSON PRC request to the `fortune1coind` you should make an HTTP POST request to an entry point:
```
http://<ip>:<port>/json_rpc
```
where:
* `<ip>` is IPv4 address of `fortune1coind` service. If the service is on local machine, use `127.0.0.1` instead of `localhost`.
* `<port>` is TCP port of `fortune1coind`. By default the service is bound to `8888`.
...
| 5.  | `get_statistics`      | Gets statistics about running `fortune1coind`.                    |
| 6.  | `get_status`          | Returns status of `fortune1coind`.                                |
...
__Input:__
```
curl -s -u user:pass -X POST http://127.0.0.1:8888/json_rpc -H 'Content-Type: application/json-rpc' -d '{
  "jsonrpc": "2.0",
  "id": "0",
...
| Field              | Type             | Description                                                           |
|--------------------|------------------|-----------------------------------------------------------------------|
| `transaction`      | `Transaction`    | Standard Transaction object. Contains info only known to `fortune1coind`. |
...
Returns misc statistics about `fortune1coind` being queried.
...
| `start_time`                           | `timestamp`    | Timestamp of `fortune1coind` start time in UTC.              |
...
| `peer_list_white`                      | `[]Peer`       | Peers `fortune1coind` has successfully connected to.         |
...
Let's make a `get_statistics` query to `fortune1coind`.
...
Get status about state of `fortune1coind`. This method supports longpolling. If you specify all input parameters, \
and they are equal to the current state of the `fortune1coind`, you will get response only when some of them change. \
...
| Field                              | Type     | Description                                                                              |
|------------------------------------|----------|------------------------------------------------------------------------------------------|
| `incoming_peer_count`              | `uint32` | Incoming peers to fortune1coind.                                                             |
| `lower_level_error`                | `string` | Error on lower level (fortune1coind for walletd, etc).                                       |
...
| `outgoing_peer_count`              | `uint32` | Outgoing peers from fortune1coind.                                                           |
...
| `top_block_height`                 | `uint32` | All transaction prior to that height have been processed by fortune1coind.                   |
...
| `first_block_timestamp`    | `timestamp` | No        | `0`           | `fortune1coind` won't return blocks earlier than this point in time. |
...
Result of call will be `broadcast`, if transaction was successfully placed into the payment queue. Note, that if `fortune1coind` is not connected to internet, this method will nevertheless succeed.
...
Let's send a previously created transaction by submitting `binary_transaction` to `fortune1coind`.
